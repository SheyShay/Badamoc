...
public class InventaireController implements Initializable{
    ...
    @FXML
    private AnchorPane InventairePage;
    ...
    void openClients(MouseEvent event) {
    	try {
			...
			InventairePage.getChildren().removeAll();
			InventairePage.getChildren().setAll(fxml);
		} catch (IOException e) {
    ...
    void inventairesInventaires(MouseEvent event) {
    	try {
			fxml = FXMLLoader.load(getClass().getResource("/interfaces/Inventaires.fxml"));
			InventairePage.getChildren().removeAll();
			InventairePage.getChildren().setAll(fxml);
		} catch (IOException e) {
			// TODO Auto-generated catch block
